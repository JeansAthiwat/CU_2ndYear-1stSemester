...
void appendmidnode(int minv,int maxv,CP::map_bst<int,int> &bst) {
    int midv = (maxv+minv)/2;

    bst[midv];//append the middle node aka root of child
    if(minv == maxv) return;// recursive Stop Condition when id = min = max
    //std::cout << "appended : " << midv << "\n";
    appendmidnode(minv,midv-1,bst);
    //std::cout << "mintomid-1 done : "<< "\n";
    appendmidnode(midv+1,maxv,bst);
    //std::cout << "midtomax-1 done : "<< "\n";
}

void gen_best_bst(int n,CP::map_bst<int,int> &bst) {
    //write your code here
    appendmidnode(1,n,bst);
}
