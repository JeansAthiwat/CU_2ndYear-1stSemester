...
class list {
  ...
  public:
    ...
    void reorder(int pos,std::vector<int> selected) {
        ...
        CP::list<T>::iterator posIt = begin();
        while(pos--) ++posIt;
        //std::cout << "pos value: " << *posIt << "\n";
        for(int i = selected.size()-1; i > 0 ; i--) {
            selected[i]  -= selected[i-1]; // make it so that the vector tell the while loop to move x step more to find the next iterator
        }
